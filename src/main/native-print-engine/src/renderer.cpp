...
RenderResult render_to_trace(
  ...
  for (const auto& page : document.pages) {
    ...
    // fires the notice.
    constexpr double page_escape_tolerance = 4.0;
    const auto escapes_page = [&page_rect](const Rect& b) {
      return b.x < page_rect.x - page_escape_tolerance ||
             b.y < page_rect.y - page_escape_tolerance ||
             b.x + b.w > page_rect.x + page_rect.w + page_escape_tolerance ||
             b.y + b.h > page_rect.y + page_rect.h + page_escape_tolerance;
    };
