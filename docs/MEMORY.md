...
## Embedded-SVG rasterizer (TODO #2 / resvg) progress (2026-05-18)
...
- **Phase 2 DONE & tested (cross-platform):** engine ferries opaque base64 SVG bytes verbatim through `EmittedCommand.svg_source` (INV-1 clean — engine never parses; INV-5 — identical bytes to both sinks). New `phase3_raster_svg_tests.cpp` case; SVG still emits the loud stub.
- **Phase 3 DONE (Windows-only, unbuilt here):** hand-owned C ABI `host/svg_rasterizer_abi.h` (ABI-version handshake, caller-allocates two-call measure/render, pinned pixel contract = straight RGBA8 R,G,B,A top-down stride w*4, host converts→premul BGRA, panic rule). `host/svg_rasterizer.{hpp,cpp}` = `ISvgRasterizer` + `SvgRasterizerDll` (LoadLibraryW, hard handshake, missing/bad → nullptr → caller emits loud stub). Fake-shim `host/test_support/fake_svg_rasterizer.c` + `tests/svg_rasterizer_abi_tests.cpp` (swap acceptance, WIN32-gated).
- **Phase 4 DONE & functionally smoke-validated:** `host/svg-rasterizer/` Rust cdylib (resvg 0.47, single dep), panic-safe (`catch_unwind` every export, `panic="unwind"`), implements the ABI exactly, system fontdb, premul→straight conversion, aspect-fit letterbox. Builds clean (`cargo build --release`); ephemeral Linux dlopen smoke: red SVG → straight RGBA `255,0,0,255`, all 4 symbols exported, handshake=1. Built separately from CMake; ship the DLL next to `print_engine_host.exe`. `target/`+`Cargo.lock` gitignored.
- **Still open (need a Windows build to validate):** Phase 5 — wire `draw_trace()` Svg branch: pass one `ISvgRasterizer*` into `draw_trace` from both preview+print call sites (INV-5), decode base64 `c.svg_source`, `render(bytes, device_box w/h, g.GetDpiX())`, composite straight-RGBA→premul-BGRA via GDI+ `DrawImage` like the Image branch; missing/failed rasterizer falls through to the existing loud Barcode||Svg stub (keep Barcode always stub). Phase 6 — determinism + SVG golden suite + ABI-symbol CI + jobLog records backend name+version. §6 escalations (loud-stub→softened-notice posture; SVG-embedded-font notice) still need spec-owner sign-off before flipping the stub off.
